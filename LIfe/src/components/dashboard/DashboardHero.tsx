    ...
    <section className="mb-5 overflow-hidden rounded-3xl border bg-card shadow-sm">
      <div className="gradient-primary p-5 text-primary-foreground">
        <div className="flex items-center gap-2 text-xs font-medium opacity-90">
          <Sparkles className="h-3.5 w-3.5" /> Próximo compromisso
        </div>
        {nextEvent ? (
          <>
            <h2 className="mt-2 text-2xl font-bold leading-tight">{nextEvent.title}</h2>
            <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm opacity-95">
              <span className="inline-flex items-center gap-1.5">
                <Clock className="h-4 w-4" />
                {fmt(nextEvent.starts_at, "EEE, dd MMM 'às' HH:mm")}
              </span>
              {nextEvent.location && (
                <span className="inline-flex items-center gap-1.5">
          ...
          <>
            <div className="mt-3 flex items-center gap-3">
              <div className={`shrink-0 overflow-hidden drop-shadow-md ${emptyAnim}`} style={{ width: `${Math.round(80 * heroScale)}px`, height: `${Math.round(80 * heroScale)}px` }}>
                <SafeImage
                  src={heroSrcFor("hero")}
